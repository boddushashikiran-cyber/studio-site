...
/* ---------------------------------------------------------
   The 3D object: an abstract distorted icosahedron that
   eases its rotation toward the cursor position, wrapped
   in a slow ambient float. Lives inside the bracketed
   "viewport" frame — the hero's signature element.
--------------------------------------------------------- */
function ReactiveShape() {
  ...
  useFrame((state) => {
    target.current.x = state.pointer.y * 0.6;
    target.current.y = state.pointer.x * 0.6;

    if (meshRef.current) {
      meshRef.current.rotation.x +=
        (target.current.x - meshRef.current.rotation.x) * 0.04;
      meshRef.current.rotation.y +=
        (target.current.y - meshRef.current.rotation.y +
          state.clock.getDelta() * 0.15) * 0.04;
    }
  });
  ...
  return (
    <section className="relative mx-auto flex min-h-screen max-w-[1400px] flex-col justify-center gap-12 px-6 pt-24 lg:flex-row lg:items-center lg:gap-8 lg:px-12">
      <div className="lg:w-[58%]">
        ...
        >
          HALFTONE STUDIO — DESIGN &amp; DEVELOPMENT
        </motion.span>
