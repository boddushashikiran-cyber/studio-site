...
// A dedicated error boundary around the WebGL canvas specifically. If
// context creation, a shader, or anything else in the R3F tree throws,
// this catches it and falls back to the settled static state instead of
...
> {
  ...
  componentDidCatch(error: unknown) {
    console.error("Hero WebGL canvas failed, falling back to static state:", error);
    this.props.onError();
  }
  render() {
...
export default function Hero() {
  ...
  const { titleField, dustField } = useMemo(() => {
    if (reducedMotion !== false) return { titleField: null, dustField: null };
    return {
  ...
  useEffect(() => {
    if (reducedMotion !== false || !titleField) return;
    ...
    // Absolute backstop: if the sequence hasn't settled within 9s for any
    // reason (a stall, not just a thrown error), force it rather than risk
    // the page being stuck on the black overlay indefinitely.
    ...
    (async () => {
      try {
        ...
        // If anything in the cinematic sequence fails unexpectedly, fail
        // safe: log it and just show the settled state immediately rather
        // than risk an unhandled error affecting the rest of the page.
        console.error("Hero cinematic sequence failed, showing settled state:", err);
        setSettled(true);
      }
    })();
      ...
      <div className={wrapperClass}>
        {!reducedMotion && titleField && dustField && (
          <div className="absolute inset-0">
            <CanvasErrorBoundary onError={() => setSettled(true)}>
